                                               ...
                                               geometry_msgs::Point rand_sample, double min_distance, boost::shared_ptr<octomap::OcTree> octree)
{
    ...
    {
        double extension = min_distance > _min_extend_range ? min_distance : _min_extend_range;
        octomap::point3d start_point(nearest_node.position.x,nearest_node.position.y, nearest_node.position.z);
        octomap::point3d end_point(nearest_node.position.x + extension * (rand_sample.x - nearest_node.position.x) / min_distance,
                                   nearest_node.position.y + extension * (rand_sample.y - nearest_node.position.y) / min_distance,
                                   nearest_node.position.z + extension * (rand_sample.z - nearest_node.position.z) / min_distance);
