...
TreeConstructor::TreeConstructor()    :
    ...
    _nodes_ordered_by_gain([this](int node1, int node2){return _rrt.nodes[node1].gain < _rrt.nodes[node2].gain;})
{
    geometry_msgs::Point rrt_seed;
    rrt_seed.x = 0.0;
    rrt_seed.y = 0.0;
    rrt_seed.z = 0.0;
    initialization(rrt_seed);
}

TreeConstructor::TreeConstructor(geometry_msgs::Point seed)   :
    _map_dimensions{0.0,0.0,0.0},
    _nodes_ordered_by_gain([this](int node1, int node2){return _rrt.nodes[node1].gain < _rrt.nodes[node2].gain;})
{
    initialization(seed);
...
void TreeConstructor::initialization(geometry_msgs::Point seed) {
    ...
    _rrt_publisher = _nh.advertise<rrt_nbv_exploration_msgs::rrt>("rrt_tree", 1000);
    _octomap_sub = _nh.subscribe("octomap_binary", 1000, &TreeConstructor::convert_octomap_msg_to_octree, this);
    _running = false;
...
void TreeConstructor::run_rrt_construction() {
    while (ros::ok())
    {
        ...
        if(_running && _map_dimensions[0] && _map_dimensions[1] && _map_dimensions[2]){
            geometry_msgs::Point rand_sample = sample_point();
            double min_distance;
            int nearest_node;
            _tree_searcher.find_nearest_neighbour(rand_sample, min_distance, nearest_node);
...
geometry_msgs::Point TreeConstructor::sample_point() {
    ...
    std::uniform_real_distribution<double> x_distribution(-_map_dimensions[0]/2,_map_dimensions[0]/2);
    std::uniform_real_distribution<double> y_distribution(-_map_dimensions[1]/2,_map_dimensions[1]/2);
    std::uniform_real_distribution<double> z_distribution(0,_map_dimensions[2]);
...
void TreeConstructor::place_new_node(geometry_msgs::Point rand_sample, double min_distance, int nearest_node) {
    //distance metric/kinematic constraints
    rrt_nbv_exploration_msgs::Node node;
    if(_collision_checker.steer(node, _rrt.nodes[nearest_node], rand_sample, min_distance, _octree))
    {
...
//change to iterations!
    if(_nodes_ordered_by_gain.size()>5) //only start exploration after constructing a few nodes to prevent blocking exploration with a suboptimal goal
    {
        ...
        case rrt_nbv_exploration_msgs::Node::ABORTED:   //update all nodes? around robot?
            ...
            _nodes_ordered_by_gain.erase(_current_goal_node);
            update_nodes(_rrt.nodes[_current_goal_node]);
            _gain_calculator.calculate_gain(_rrt.nodes[_current_goal_node], _octree);
            _nodes_ordered_by_gain.insert(_current_goal_node);
            update_current_goal();
            break;
...
void TreeConstructor::update_nodes(rrt_nbv_exploration_msgs::Node& center_node)
{
    ROS_INFO("start updating nodes");
    std::vector<int> updatable_nodes = _tree_searcher.search_in_radius(center_node.position, _radius_search_range);
...
void TreeConstructor::convert_octomap_msg_to_octree(const octomap_msgs::Octomap::ConstPtr& map_msg)
{
...
{
 _octree->getMetricSize(_map_dimensions[0], _map_dimensions[1], _map_dimensions[2]);
}
}
