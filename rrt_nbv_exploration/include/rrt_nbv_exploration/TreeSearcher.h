...
private:
	...
	struct rrt_adaptor {
		...
		inline size_t kdtree_get_point_count() const {
			return rrt.node_counter;
		}
		inline double kdtree_get_pt(const size_t idx, const size_t dim) const {
			if (dim == 0)
				return rrt.nodes[idx].position.x;
			else if (dim == 1)
				return rrt.nodes[idx].position.y;
			else
	...
	typedef nanoflann::KDTreeSingleIndexAdaptor<
			nanoflann::L2_Simple_Adaptor<double, rrt_adaptor>, rrt_adaptor, 3> kd_tree_adaptor;
