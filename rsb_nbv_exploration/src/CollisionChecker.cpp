...
void CollisionChecker::initialize(rsb_nbv_exploration_msgs::Graph &rrg,
...
bool CollisionChecker::steer(rsb_nbv_exploration_msgs::Graph &rrg,
	...
	if (!isCircleInCollision(rand_sample.x, rand_sample.y, map,
		...
		new_node.position.y = rand_sample.y;
		new_node.position.z = rand_sample.z;
		new_node.status = rsb_nbv_exploration_msgs::Node::INITIAL;
		...
		double height = 0;
		std::vector<rsb_nbv_exploration_msgs::Edge> new_edges;
		std::vector<rsb_nbv_exploration_msgs::Edge> new_retriable_edges;
		std::vector<visualization_msgs::Marker> new_edge_markers;
		for (auto it : nodes) {
			if (rrg.nodes[it.first].status
					== rsb_nbv_exploration_msgs::Node::INACTIVE) // skip connections to inactive nodes
				continue;
			...
			if (edge_length > _max_edge_distance) // path box only allowed up to a given length
				...
				continue;
			} else if (_inflation_active && !unmovable_point
			...
			if (edge_free) {
				...
				rsb_nbv_exploration_msgs::Edge new_edge = constructEdge(
...
std::vector<int> CollisionChecker::inflateExistingNode(
	...
	if (new_radius > rrg.nodes[node].radius) {
		...
		for (auto it : nodes) {
			double distance = sqrt(it.second);
			if (rrg.nodes.at(it.first).gain == 0
					&& new_radius > distance + rrg.nodes.at(it.first).radius) { // node is completely engulfed by inflated node's new radius
			...
			if (!calculateEdge(it.first, edge_length, distance, edge_center,
				...
				rsb_nbv_exploration_msgs::Edge new_edge;
...
int CollisionChecker::collisionCheckForFailedNode(
		rsb_nbv_exploration_msgs::Graph &rrg, int node, bool inflate) {
	nav_msgs::OccupancyGrid map = *_occupancy_grid;
	std::vector<int8_t> tmp_vis_map_data = _vis_map.data;
...
void CollisionChecker::checkNewEdgePathLength(
		...
		rsb_nbv_exploration_msgs::Graph &rrg,
		rsb_nbv_exploration_msgs::Edge &edge, std::list<int> &nodes_to_update,
		bool &added_node_to_update) {
...
void CollisionChecker::retryEdges(rsb_nbv_exploration_msgs::Graph &rrg,
		geometry_msgs::Pose robot_pos, std::list<int> &nodes_to_update,
		bool &added_node_to_update) {
	nav_msgs::OccupancyGrid map = *_occupancy_grid;
	_vis_map.header.stamp = ros::Time::now();
...
bool CollisionChecker::checkMovementWithNearestNode(double &x, double &y,
		int &direction, rsb_nbv_exploration_msgs::Node &nearest_node,
...
rsb_nbv_exploration_msgs::Edge CollisionChecker::constructEdge(double edge_yaw,
...
bool CollisionChecker::checkEngulfing(geometry_msgs::Point &point,
		int &nearest_node, rsb_nbv_exploration_msgs::Graph &rrg) {
...
void CollisionChecker::findBestConnectionForNode(
		rsb_nbv_exploration_msgs::Graph &rrg,
		rsb_nbv_exploration_msgs::Node &node, geometry_msgs::Pose &robot_pos,
		bool new_node, std::vector<rsb_nbv_exploration_msgs::Edge> edges) {
	std::vector<rsb_nbv_exploration_msgs::Edge> node_edges;
	...
	for (auto &edge : node_edges) {
		...
		if (rrg.nodes[neighbor_node_index].status
			...
			rsb_nbv_exploration_msgs::Node updated_node; // calculate potential cost if connected via this edge
...
void CollisionChecker::insertNodeInRrg(rsb_nbv_exploration_msgs::Node &node,
		rsb_nbv_exploration_msgs::Graph &rrg) {
	if (!_available_nodes.empty()) {
...
void CollisionChecker::insertEdgeInRrg(rsb_nbv_exploration_msgs::Edge &edge,
		rsb_nbv_exploration_msgs::Graph &rrg) {
...
void CollisionChecker::removeRetriableEdgesForNode(int node) {
	_retriable_edges.erase(
			std::remove_if(_retriable_edges.begin(), _retriable_edges.end(),
					[node](rsb_nbv_exploration_msgs::Edge &edge) {
...
bool CollisionChecker::checkConnectionToFrontierPathWaypoint(
		rsb_nbv_exploration_msgs::Graph &rrg, int node,
	...
	double edge_yaw = atan2(waypoint.y - rrg.nodes[node].position.y,
			waypoint.x - rrg.nodes[node].position.x);
	fmod(edge_yaw, M_PI / 2) == 0 ?
...
void CollisionChecker::visualizeNode(rsb_nbv_exploration_msgs::Node &node,
		int failed) {
	visualization_msgs::Marker node_point;
	node_point.header.frame_id = "/map";
	node_point.header.stamp = ros::Time();
