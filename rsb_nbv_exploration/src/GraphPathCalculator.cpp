...
void GraphPathCalculator::updatePathsToRobot(int start_node,
		rsb_nbv_exploration_msgs::Graph &rrg, geometry_msgs::Pose robot_pos,
	...
	if (distance_to_nearest_node_squared <= _robot_radius_squared) { // robot in proximity to nearest node
		...
		if (next_node != -1 && edge_to_next_node != -1
				&& rrg.nodes[next_node].status
						!= rsb_nbv_exploration_msgs::Node::FAILED) { // ignore start node for distance, path, traversability and radii
...
void GraphPathCalculator::findBestRoutes(
		...
		rsb_nbv_exploration_msgs::Graph &rrg, bool reset,
		std::list<int> &nodes_to_update, bool &added_node_to_update) {
			...
			if (!rrg.edges.at(edge).inactive) {
				...
				if (isNodeInPath(neighbor_node_index, current_node, rrg)
						|| rrg.nodes[neighbor_node_index].status
								== rsb_nbv_exploration_msgs::Node::FAILED) { // don't allow loops and failed nodes in path
					continue;
				}
				rsb_nbv_exploration_msgs::Node node_with_new_connection =
				...
				if (node_with_new_connection.cost_function
					...
					addToNodeQueue(node_queue, neighbor_node_index,
...
bool GraphPathCalculator::isNodeInPath(int neighbor_node_index,
		int current_node, rsb_nbv_exploration_msgs::Graph &rrg) {
...
rsb_nbv_exploration_msgs::Node GraphPathCalculator::calculateCostFunctionForConnection(
		int neighbor_node, int node, int edge,
		rsb_nbv_exploration_msgs::Graph &rrg) {
	rsb_nbv_exploration_msgs::Node updated_node;
...
int GraphPathCalculator::findExistingEdge(rsb_nbv_exploration_msgs::Graph &rrg,
...
void GraphPathCalculator::getLocalNavigationPath(
		...
		rsb_nbv_exploration_msgs::Graph &rrg, int goal_node,
...
bool GraphPathCalculator::getHeadingBetweenNodes(int start_node, int end_node,
		int &yaw, double &distance, rsb_nbv_exploration_msgs::Graph &rrg) {
...
int GraphPathCalculator::determineGoalYaw(int current_goal,
		rsb_nbv_exploration_msgs::Graph &rrg, geometry_msgs::Point robot_pose,
		bool homing) {
...
bool GraphPathCalculator::neighbourNodes(rsb_nbv_exploration_msgs::Graph &rrg,
...
std::map<int, int> GraphPathCalculator::findShortestRoutes(
		rsb_nbv_exploration_msgs::Graph &rrg, int target_connecting_node,
	...
	for (auto node : rrg.nodes) {
		local_nodes.emplace_back(node.index,
				node.status == rsb_nbv_exploration_msgs::Node::INACTIVE);
	}
...
void GraphPathCalculator::addToNodeQueue(
		std::set<std::pair<double, int> > &node_queue, int neighbor_node_index,
		double cost) {
	auto it = std::find_if(node_queue.begin(), node_queue.end(),
			[neighbor_node_index](std::pair<double, int> pair) {
				return pair.second == neighbor_node_index;
			});
	if (it == node_queue.end() || it->first > cost) {
		node_queue.insert(std::make_pair(cost, neighbor_node_index));
		if (it->first > cost) {
			node_queue.erase(it);
		}
	}
