...
AedeInterface::AedeInterface() :
	...
	_request_goal_service = nh.serviceClient<
			rsb_nbv_exploration_msgs::RequestGoal>("requestGoal");
...
void AedeInterface::waypointReached() {
	if (_current_plan.size() <= 1) { //last waypoint in plan=goal
		...
		updateCurrentGoal(rsb_nbv_exploration_msgs::Node::VISITED);
		_follows_goal = false;
	} else {
...
void AedeInterface::checkIfRobotMoved() {
	...
	if (!robot_moved) {
		if (!_idle_timer_fired_on_goal && _current_plan.size() > 1) {
			...
			_current_plan.clear();
			_follows_goal = false;
			updateCurrentGoal(rsb_nbv_exploration_msgs::Node::FAILED);
		}
	}
}
...
void AedeInterface::requestPath() {
	rsb_nbv_exploration_msgs::RequestGoal goal;
...
void AedeInterface::explorationGoalObsoleteCallback(
	...
	if (exploration_goal_obsolete->goal_obsolete) {
		updateCurrentGoal(rsb_nbv_exploration_msgs::Node::ABORTED);
		_follows_goal = false;
		_current_plan.clear();
	}
}
