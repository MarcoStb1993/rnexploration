 ...
 * nearest neighbor search as well as radius search in the global graph. It always treats all frontier
...
public:
	 ...
	 * @param GG with frontiers that act as nodes for the kd tree
	 */
	void initialize(rrg_nbv_exploration_msgs::GlobalGraph &gg);
	/**
	 * @brief Rebuilds the kd-tree if necessary
	 * @param GG with frontiers that act as nodes for the kd tree
	 */
	void rebuildIndex(rrg_nbv_exploration_msgs::GlobalGraph &gg);
	/**
	 * @brief Find the nearest neighbor in the current kd-tree to the point given as parameter
	 * @param 3D point
	 * @param Reference to minimum squared distance calculated between a frontier in the tree and the point
	 * @param Reference to the nearest frontier in the tree from the given point
	 */
	void findNearestNeighbour(geometry_msgs::Point point, double &min_distance,
			int &nearest_node);
	/**
	 * @brief Finds all frontiers in the current kd-tree in the given (squared) radius around a provided
	 * point, radius is squared because Euclidean distance is squared as well
	 * @param 3D Point
	 * @param Search radius (needs to be squared)
	 * @return List of the indices of all frontiers in this radius and their respective squared distance
...
private:
	/**
	 * @brief The gg_adaptor struct serves as an adaptor for nanoflann, using the nodes in gg
	 */
	struct gg_adaptor {
		rrg_nbv_exploration_msgs::GlobalGraph gg;
		inline size_t kdtree_get_point_count() const {
			return gg.frontiers_counter;
		}
		inline double kdtree_get_pt(const size_t idx, const size_t dim) const {
			if (dim == 0)
				return gg.frontiers[idx].viewpoint.x;
			else if (dim == 1)
				return gg.frontiers[idx].viewpoint.y;
			else
				return 0; //gg.frontiers[idx].viewpoint.z;
		}
		template<class BBOX>
