...
namespace rrg_nbv_exploration
{
	...
	CollisionChecker::CollisionChecker()
	{
		...
		_path_box_distance_thres = 2 * sqrt(pow(_robot_radius, 2) - pow(_robot_width / 2, 2));
		_init_vis_map = false;
									  ...
									  std::shared_ptr<GraphPathCalculator> graph_path_calculator)
	{
		...
		initVisMap(map);
		if (!_node_edges.markers.empty())
		{ // remove all visualized markers
												...
												rrg_nbv_exploration_msgs::Graph &rrg)
	{
		...
		if (_inflation_active && !isCircleInCollision(rrg.nodes[0].position.x,
		...
		{ // no inflation active, do not check collision for root
		...
		double radius)
	{
		...
		while (sx >= mx)
		{
			...
			while (sx > mx && radius_squared < (pow(sy - my - half_resolution, 2) + pow(sx - mx - half_resolution, 2)))
			{
								 ...
								 bool unmovable_point)
	{
		...
		if (!(_inflation_active ? checkEngulfing(rand_sample, nearest_node, rrg) : checkDistance(rand_sample, nearest_node, rrg)) && !unmovable_point)
		{
								 ...
								 tmp_vis_map_data, node_cost, node_tiles, node_collision))
		{
			...
			std::vector<std::pair<int, double>> nodes =
				_graph_searcher->searchInRadius(new_node.position,
												_inflation_active ? pow(_sensor_range + rrg.largest_node_radius,
																		2)
			...
			for (auto it : nodes)
			{
				if (rrg.nodes[it.first].status == rrg_nbv_exploration_msgs::Node::INACTIVE) // skip connections to inactive nodes
					continue;
				...
				{ // only connect nodes with enough radius overlap
					continue;
				}
				else if (_inflation_active && !unmovable_point && it.second < rrg.nodes[it.first].squared_radius)
				{
					return false; // new node position engulfed by this node's radius
				}
				bool edge_free =
					check_required ? fmod(edge_yaw, M_PI / 2) == 0 ? !isAlignedRectangleInCollision(
				...
				else if (edge_collision == Collisions::unknown)
				{ // store failed edge for later reassessment
					rrg_nbv_exploration_msgs::Edge new_edge = constructEdge(
						edge_yaw, edge_length, edge_cost, edge_tiles, it.first,
						new_node.index, distance, rrg);
			...
			if (connected)
			{
				...
				for (auto edge : new_retriable_edges)
				{ // add edges which failed because of unknown space
														   ...
														   std::list<int> &nodes_to_update, bool &added_node_to_update)
	{
		...
		if (new_radius > rrg.nodes[node].radius)
		{
			...
			{
				double distance = sqrt(it.second);
				if (rrg.nodes.at(it.first).gain == 0 && new_radius > distance + rrg.nodes.at(it.first).radius)
				{ // node is completely engulfed by inflated node's new radius
					engulfed_nodes.push_back(it.first);
				}
				if (it.first == node || _graph_path_calculator->findExistingEdge(rrg, node, it.first) != -1 || rrg.nodes[it.first].status == rrg_nbv_exploration_msgs::Node::INACTIVE)
				...
				{
					double edge_yaw = atan2(
						rrg.nodes[node].position.y - rrg.nodes[it.first].position.y,
						rrg.nodes[node].position.x - rrg.nodes[it.first].position.x);
	...
	{
		if (rrg.nodes[edge.first_node].distance_to_robot + edge.length < rrg.nodes[edge.second_node].distance_to_robot)
		{
			_graph_path_calculator->updatePathsToRobot(edge.first_node, rrg,
													   robot_pos, false, nodes_to_update, added_node_to_update);
		}
		else if (rrg.nodes[edge.second_node].distance_to_robot + edge.length < rrg.nodes[edge.first_node].distance_to_robot)
		{
		...
		_retriable_edges.erase(
			std::remove_if(_retriable_edges.begin(), _retriable_edges.end(),
							   ...
							   rrg_nbv_exploration_msgs::Edge &edge)
						   {
							   int edge_cost = 0, edge_tiles = 0, edge_collision = Collisions::empty;
							   geometry_msgs::Point edge_center;
							   edge_center.x = (rrg.nodes[edge.first_node].position.x + rrg.nodes[edge.second_node].position.x) / 2;
							   edge_center.y = (rrg.nodes[edge.first_node].position.y + rrg.nodes[edge.second_node].position.y) / 2;
							   double edge_length = edge.length - _path_box_distance_thres;
							   fmod(edge.yaw, M_PI / 2) == 0 ? isAlignedRectangleInCollision(edge_center.x,
							   ...
							   else if (edge_collision == Collisions::occupied)
							   { // discard edge
								   return true;
							   }
	...
	void CollisionChecker::calculateNextInflatedCircleLinesOffset()
	{
		...
		{																										   // first inflation ring offset
			new_radius = ceil(_robot_radius / _grid_map_resolution) * _grid_map_resolution + _grid_map_resolution; // round up radius to next full grid map tile
			prevOffsetSet = _circle_lines_offset;
		}
		else if (_inflated_ring_lines_offsets.back().first + _grid_map_resolution > _local_graph_radius)
		{
			return; // abort, all inflation ring offsets are calculated
		}
		else
		{ // new inflation ring
			new_radius = _inflated_ring_lines_offsets.back().first + _grid_map_resolution;
			prevOffsetSet = _inflated_ring_lines_offsets.back().second;
		}
		std::vector<CircleLine> newOffsetSet = calculateCircleLinesOffset(
			new_radius);
		for (int i = 0; i < prevOffsetSet.size(); i++)
		{ // prev offset set size must be smaller equals new offset set
										   ...
										   int &cost, int &tiles, int &collision, bool recalculate)
	{
		...
		for (auto it : offsets)
		{ // always starts at highest x offset and ends at highest y offset
			if (recalculate || it.x_start == 0)
			{ // circle
				...
				if (it.y_offset != 0)
				{ // y offset is only 0 in the center slice, otherwise symmetrical
					if (isLineInCollision(map_x - it.x_offset, map_x + it.x_offset,
			...
			{ // ring
				...
				if (isLineInCollision(map_x + it.x_start, map_x + it.x_offset,
					...
					if (!checkDirection(direction, fixed_direction,
										it.x_offset == max_x_offset ? Directions::south : it.y_offset == max_y_offset ? Directions::east
				...
				if (isLineInCollision(map_x - it.x_offset, map_x - it.x_start,
					...
					if (!checkDirection(direction, fixed_direction,
										it.x_offset == max_x_offset ? Directions::north : it.y_offset == max_y_offset ? Directions::east
				...
				{ // y offset is only 0 in the center slice, otherwise symmetrical
					if (isLineInCollision(map_x + it.x_start, map_x + it.x_offset,
						...
						if (!checkDirection(direction, fixed_direction,
											it.x_offset == max_x_offset ? Directions::south : it.y_offset == max_y_offset ? Directions::west
					...
					if (isLineInCollision(map_x - it.x_offset, map_x - it.x_start,
						...
						if (!checkDirection(direction, fixed_direction,
											it.x_offset == max_x_offset ? Directions::north : it.y_offset == max_y_offset ? Directions::west
		...
		{ // only store cost, tiles and visualization when no collision was detected
			cost = new_cost;
			tiles = new_tiles;
			vis_map = tmp_vis_map;
		}
	...
	bool CollisionChecker::checkDirection(int &current_direction, bool &fixed,
										  int new_direction)
	{
		if (current_direction == Directions::center)
		...
		{ // merge directions
			...
			if (fixed || difference == 45)
			{ // keep diagonal direction
				current_direction =
					(current_direction == 0 || (current_direction % 90) == 0) ? new_direction : current_direction;
			}
														...
														bool &fixed_direction)
	{
		...
		double distance_squared = sqrt(
			pow(new_point.x - nearest_node.position.x, 2) + pow(new_point.y - nearest_node.position.y, 2));
		...
		{
			distance_threshold = _path_box_distance_thres / 2 + _path_box_distance_thresholds.at(
																								 (nearest_node.radius - _robot_radius) / _grid_map_resolution)
																	.second;
		...
		if (move_from_node || move_towards_node)
		{
			...
			if (node_direction < 0)
				node_direction += 360;
			int direction_from_nearest_node = ((int)round(node_direction / 45)) * 45;
			if (direction_from_nearest_node == 360)
				direction_from_nearest_node = 0;
			if (move_towards_node)
			{ // opposite direction
				direction_from_nearest_node =
					direction_from_nearest_node + direction_from_nearest_node > 180 ? (-180) : 180;
			}
			if (!checkDirection(direction, fixed_direction,
								direction_from_nearest_node))
			{ // merge directions, if it fails, abort inflation
				return false;
			}
			else
			{ // check if merged direction is acceptable
				geometry_msgs::Point merged_point = movePoint(direction, x, y);
				double distance_merged_squared = sqrt(
					pow(merged_point.x - nearest_node.position.x, 2) + pow(merged_point.y - nearest_node.position.y, 2));
				double distance_merged = sqrt(distance_merged_squared);
				double distance_merged_to_radius = distance_merged - nearest_node.radius;
				if (distance_merged_to_radius < 0 || distance_merged > distance_threshold)
				{
					return false;
				}
			}
			...
			else
			{
				if (move_node)
				{
					if (direction == Directions::center)
					{
						...
						if (it == --_inflated_ring_lines_offsets.end())
						{ // calculate next inflation ring if not reached sensor range
							calculateNextInflatedCircleLinesOffset();
						}
						index++;
					...
					else
					{
						if (!checkMovementWithNearestNode(x, y, direction,
							...
							!moveCircle(x, y, direction, index,
										...
										tiles, collision))
						{ // don't increment iterator, repeat this inflation for new position next iteration
							return current_radius;
						}
					}
				}
									  ...
									  int &tiles, int &collision)
	{
		...
		if (isSetInCollision(new_point.x, new_point.y, fixed_direction, map,
							 vis_map,
							 ring == 0 ? _circle_lines_offset : _inflated_ring_lines_offsets.at(ring - 1).second, cost,
												  ...
												  std::vector<int8_t> &vis_map, int &cost, int &tiles, int &collision)
	{
		...
		if (yaw > 0 && yaw < PI_HALF)
		{																			   // height from top left to bottom right
			map_corners[0] = {x + height_cos - width_sin, y + width_cos + height_sin}; // left corner
			map_corners[1] = {x - height_cos - width_sin, y + width_cos - height_sin}; // bottom corner
			map_corners[2] = {x + height_cos + width_sin, y - width_cos + height_sin}; // top corner
			map_corners[3] = {x - height_cos + width_sin, y - width_cos - height_sin}; // right corner
		}
		else
		{																			   // yaw >  -PI_HALF && yaw < 0 (height from bottom left to top right)
			map_corners[0] = {x - height_cos - width_sin, y + width_cos - height_sin}; // left corner
			map_corners[1] = {x - height_cos + width_sin, y - width_cos - height_sin}; // bottom corner
			map_corners[2] = {x + height_cos - width_sin, y + width_cos + height_sin}; // top corner
			map_corners[3] = {x + height_cos + width_sin, y - width_cos + height_sin}; // right corner
		}
		double gradient_up = (map_corners[2].x - map_corners[3].x) / (map_corners[2].y - map_corners[3].y);
		...
		double iterator_y = (floor(map_corners[3].y / _grid_map_resolution) + 1.0) * _grid_map_resolution;
		double iterator_x = (floor(map_corners[3].x / _grid_map_resolution) + 0.5) * _grid_map_resolution;
		double iterator_x_top = map_corners[3].x + gradient_top * (iterator_y - map_corners[3].y);
		double offset_x_top = (iterator_x_top - iterator_x) / _grid_map_resolution;
		double iterator_x_bot = map_corners[3].x + gradient_bot * (iterator_y - map_corners[3].y);
		...
		{
			int x_start, x_end;
			if (!top_corner_reached && iterator_y >= map_corners[2].y)
			{ // top corner reached, change gradient
				gradient_top = gradient_down;
				iterator_x_top = map_corners[2].x + gradient_top * (iterator_y - map_corners[2].y - _grid_map_resolution);
			...
			if (!bot_corner_reached && iterator_y >= map_corners[1].y)
			{ // bot corner reached, change gradient
				gradient_bot = gradient_up;
				iterator_x_bot = map_corners[1].x + gradient_bot * (iterator_y - map_corners[1].y - _grid_map_resolution);
														 ...
														 int &tiles, int &collision)
	{
		...
		{														// yaw == -M_PI || 0 || M_PI (height in x direction)
	...
	{
		rand_sample.x = (round(rand_sample.x / _grid_map_resolution) + 0.5) * _grid_map_resolution;
		rand_sample.y = (round(rand_sample.y / _grid_map_resolution) + 0.5) * _grid_map_resolution;
	}
										 ...
										 rrg_nbv_exploration_msgs::Graph &rrg)
	{
		if (_inflation_active)
		{
			...
			double nearest_node_path_box_distance = sqrt(
				rrg.nodes[nearest_node].squared_radius - pow(_robot_width / 2, 2));
			edge_length = distance - new_node_path_box_distance - nearest_node_path_box_distance;
			if (edge_length <= 0)
			{
				edge_length = 0;
				return false;
		...
		else
		{
			...
			edge_center.x = (rrg.nodes[nearest_node].position.x + new_node.position.x) / 2;
			edge_center.y = (rrg.nodes[nearest_node].position.y + new_node.position.y) / 2;
		}
		return true;
	}
										  ...
										  int &nearest_node, rrg_nbv_exploration_msgs::Graph &rrg)
	{
		...
		if (nearest_node == -1)
		{ // if no nearby nodes found, find nearest neighbor to move sample closer
			_graph_searcher->findNearestNeighbour(point, min_distance,
												  nearest_node);
			min_distance = sqrt(min_distance);
		}
		int index = ((rrg.nodes[nearest_node].radius - _robot_radius) / _grid_map_resolution) - 1;
		double distance_threshold;
		distance_threshold =
			index >= 0 ? _path_box_distance_thres / 2 + _path_box_distance_thresholds.at(index).second : _path_box_distance_thres;
		if (min_distance > distance_threshold)
		{ // move sample point closer to nearest node to enable connection
			point.x = rrg.nodes[nearest_node].position.x - (distance_threshold * (rrg.nodes[nearest_node].position.x - point.x) / min_distance);
			point.y = rrg.nodes[nearest_node].position.y - (distance_threshold * (rrg.nodes[nearest_node].position.y - point.y) / min_distance);
		}
		return true;
	}
										 ...
										 int &nearest_node, rrg_nbv_exploration_msgs::Graph &rrg)
	{
		...
		if (min_distance >= _min_edge_distance_squared)
		{
			if (min_distance >= _max_edge_distance_squared)
			{
				...
				point.x = rrg.nodes[nearest_node].position.x - (_max_edge_distance * (rrg.nodes[nearest_node].position.x - point.x) / distance);
				point.y = rrg.nodes[nearest_node].position.y - (_max_edge_distance * (rrg.nodes[nearest_node].position.y - point.y) / distance);
			}
			return true;
		}
		return false;
		...
		bool new_node, std::vector<rrg_nbv_exploration_msgs::Edge> edges)
	{
		...
		if (!new_node)
		{ // get edges from given existing node
		...
		{ // use given edges for new node
			node_edges = edges;
		}
		for (auto &edge : node_edges)
		{
			int neighbor_node_index =
				edge.first_node == node.index ? edge.second_node : edge.first_node;
			if (new_node)
			{ // store edges in RRG and references in the particular nodes
				insertEdgeInRrg(edge, rrg);
			...
			if (rrg.nodes[neighbor_node_index].status != rrg_nbv_exploration_msgs::Node::FAILED && !std::isinf(rrg.nodes[neighbor_node_index].cost_function))
			{
				...
				updated_node.distance_to_robot =
					rrg.nodes[neighbor_node_index].distance_to_robot + edge.length;
				...
				if (neighbor_node_index == rrg.nearest_node)
				{
					...
					int yaw_to_start_node =
						(int)(((atan2(node.position.y - robot_pos.position.y,
									  node.position.x - robot_pos.position.x) *
								180.0 / M_PI)));
				...
				updated_node.heading_change_to_robot =
					rrg.nodes[neighbor_node_index].heading_change_to_robot + _graph_path_calculator->getAbsoluteAngleDiff(
				...
				updated_node.traversability_cost_to_robot =
					rrg.nodes[neighbor_node_index].traversability_cost_to_robot + node.traversability_cost + edge.traversability_cost;
				updated_node.traversability_weight_to_robot =
					rrg.nodes[neighbor_node_index].traversability_weight_to_robot + node.traversability_weight + edge.traversability_weight;
	...
	void CollisionChecker::removeRetriableEdgesForNode(int node)
	{
		_retriable_edges.erase(
			std::remove_if(_retriable_edges.begin(), _retriable_edges.end(),
						   ...
						   {
							   return (edge.first_node == node || edge.second_node == node); // remove retriable edge if connecting to the newly inactive node
						   }),
			_retriable_edges.end());
	}
		...
		geometry_msgs::Point frontier, double distance)
	{
		...
		fmod(edge_yaw, M_PI / 2) == 0 ? isAlignedRectangleInCollision(edge_center.x, edge_center.y,
