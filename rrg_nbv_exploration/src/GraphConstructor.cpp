...
namespace rrg_nbv_exploration
{
	...
	void GraphConstructor::initialization(geometry_msgs::Point seed)
	{
		...
		_local_sampling_radius = std::min(_local_graph_radius, _local_sampling_radius); // cannot exceed local graph radius
		...
		_local_sampling_radius = std::min(_local_sampling_radius, _local_graph_radius); // cannot sample outside local area
	}
	...
	void GraphConstructor::runExploration()
	{
		...
		if (_running)
		{
			...
			if (_local_running && _map_min_bounding[0] && _map_min_bounding[1] && _map_min_bounding[2])
			{
				...
				{ // check if robot heading changed and update heading
	...
	void GraphConstructor::expandGraph(bool update_paths)
	{
		...
		for (auto new_node_position : _new_node_positions)
		{ // add stored positions
										 ...
										 bool unmovable_point)
	{
		...
		if (sampling_success && _collision_checker->steer(_rrg, node, rand_sample, _robot_pose,
														  unmovable_point))
		{
			if (!std::isinf(node.cost_function))
			{ // do not update unreachable nodes
				if (update_paths)
											   ...
											   geometry_msgs::Point center, double radius)
	{
		...
		return (rand_sample.x >= _map_min_bounding[0] && rand_sample.x <= _map_max_bounding[0] && rand_sample.y >= _map_min_bounding[1] && rand_sample.y <= _map_max_bounding[1]);
	}
	...
	void GraphConstructor::pruneLocalGraph()
	{
		...
		{ // start path update from each connected nodes
		...
		for (auto node : disconnected_nodes)
		{
			...
			{ // check if disconnected nodes are still disconnected and deactivate them
				//				ROS_INFO_STREAM(
		...
		std::set<int> &pruned_nodes, std::set<int> &pruned_edges)
	{
		...
		for (auto &node : _rrg.nodes)
		{
			if (node.status != rrg_nbv_exploration_msgs::Node::INACTIVE)
			{
				...
				if (distance_squared > _local_graph_radius_squared)
				{ // remove node from local graph
					...
					for (int edge : node.edges)
					{
						...
						pruned_edges.insert(edge);
						int remaining_node =
							_rrg.edges[edge].first_node == node.index ? _rrg.edges[edge].second_node : _rrg.edges[edge].first_node;
						//					ROS_INFO_STREAM(
	...
	{
		return _rrg.nodes[neighbor_node].path_to_robot.size() > 1 && _rrg.nodes[neighbor_node].path_to_robot.at(
	...
	void GraphConstructor::handlePrunedEdges(const std::set<int> &pruned_edges)
	{
		...
		while (pruned_edge != pruned_edges.rend())
		{ // remove or add inactive edges
			if (*pruned_edge == _rrg.edge_counter - 1)
			{
				int next_pruned_edge =
					std::next(pruned_edge) == pruned_edges.rend() ? -1 : *std::next(pruned_edge); // if last pruned edge, try to remove inactive edges up to index 0
	...
	void GraphConstructor::handlePrunedNodes(const std::set<int> &pruned_nodes)
	{
		...
		std::sort(nodes.begin(), nodes.end(), [this](int node_one, int node_two)
				  { return sortByPathLength(node_one, node_two); }); // sort ascending by path to robot length

		findConnectedNodesWithGain(nodes);
		for (int i = nodes.size() - 1; i >= 0; i--)
		{ // deactivate nodes by path length to merge and continue paths correctly
			deactivateNode(nodes.at(i));
		}
		bool removed_nodes = false;
		auto pruned_node = pruned_nodes.rbegin();
		while (pruned_node != pruned_nodes.rend())
		{ // remove or add inactive edges
			if (*pruned_node == _rrg.node_counter - 1)
			{
				int next_pruned_node =
					std::next(pruned_node) == pruned_nodes.rend() ? -1 : *std::next(pruned_node); // if last pruned node, try to remove inactive nodes up to root
				for (int i = *pruned_node; i > next_pruned_node; i--)
				{
					if (_rrg.nodes.at(i).status == rrg_nbv_exploration_msgs::Node::INACTIVE)
					{
						ROS_INFO_STREAM(
	...
	{
		return _rrg.nodes[node_one].path_to_robot.size() < _rrg.nodes[node_two].path_to_robot.size();
	}
	...
	void GraphConstructor::findConnectedNodesWithGain(std::vector<int> &nodes)
	{
		...
		while (!possible_frontiers.empty())
		{
			...
			{ // build frontier from possible frontier nodes
				ROS_INFO_STREAM(
					"Look at node " << frontier.at(i) << " in frontier with size " << frontier.size());
				for (auto edge : _rrg.nodes.at(frontier.at(i)).edges)
				{
					int neighbor_node =
						frontier.at(i) == _rrg.edges.at(edge).first_node ? _rrg.edges.at(edge).second_node : _rrg.edges.at(edge).first_node;
					if (_rrg.nodes.at(neighbor_node).gain > 0 && possible_frontiers.erase(neighbor_node) > 0)
					{ // neighbor will also become a frontier
						ROS_INFO_STREAM(
			...
			{
				if (_rrg.nodes.at(frontier.at(i)).distance_to_robot < shortest_distance_to_robot)
				{
		...
		std::vector<int> &disconnected_nodes, std::set<int> &connected_nodes)
	{
		for (int i = 0; i < disconnected_nodes.size(); i++)
		{
			...
			{
				int neighbor_node =
					_rrg.edges[edge].first_node == node ? _rrg.edges[edge].second_node : _rrg.edges[edge].first_node;
				//			ROS_INFO_STREAM(
				...
				if (isNextInPathToRobot(neighbor_node, node))
				{
					...
					if (std::find(disconnected_nodes.begin(),
								  disconnected_nodes.end(), neighbor_node) == disconnected_nodes.end())
					{ // only add neighbor if not already present
						disconnected_nodes.push_back(neighbor_node);
					}
					connected_nodes.erase(neighbor_node);
				}
				else if (!_rrg.edges[edge].inactive && !std::isinf(_rrg.nodes[neighbor_node].cost_function))
				{
					//				ROS_INFO_STREAM(
	...
	{
		if (_current_goal_node >= 0 && _rrg.nodes[_current_goal_node].path_to_robot.size() > 1)
		{
	...
	{
		if (pos.x != _last_robot_pos.x || pos.y != _last_robot_pos.y || pos.z != _last_robot_pos.z)
		{
			_last_robot_pos = pos;
			double min_distance;
			int nearest_node;
			_graph_searcher->findNearestNeighbour(pos, min_distance, nearest_node);
			if (nearest_node != _rrg.nearest_node)
			{ // if nearest node changed
				if (_next_node != -1 && nearest_node != _next_node)
				{
					...
					double projection_on_edge =
						((pos.x - _rrg.nodes[_rrg.nearest_node].position.x) * (_rrg.nodes[_next_node].position.x - _rrg.nodes[_rrg.nearest_node].position.x) + (pos.y - _rrg.nodes[_rrg.nearest_node].position.y) * (_rrg.nodes[_next_node].position.y - _rrg.nodes[_rrg.nearest_node].position.y)) / pow(_rrg.edges[_edge_to_next_node].length, 2);
					if (projection_on_edge > 0 && projection_on_edge < 1)
					{ // if projection is outside these bounds, the robot is outside the line segment
						double distance_to_edge =
							pow(
								(pos.x - (_rrg.nodes[_rrg.nearest_node].position.x + projection_on_edge * (_rrg.nodes[_next_node].position.x - _rrg.nodes[_rrg.nearest_node].position.x))),
								2) +
							pow(
								(pos.y - (_rrg.nodes[_rrg.nearest_node].position.y + projection_on_edge * (_rrg.nodes[_next_node].position.y - _rrg.nodes[_rrg.nearest_node].position.y))),
								2);
						if (distance_to_edge <= min_distance)
						{ // robot is closer to edge than to nearest node, keep current one
							return false;
						}
					}
				}
			...
			{ // nearest node remained the same, just update distance
				double recalculate_distance = _robot_radius_squared / SQRT10;
				if (nearest_node == _current_goal_node && _distance_to_nearest_node_squared > recalculate_distance && min_distance <= recalculate_distance)
				{ // update current goal node gain when 1/10 robot radius away
					_nodes_to_reupdate.clear();
	...
	void GraphConstructor::publishExplorationGoalObsolete()
	{
		...
		if ((!_pursuing_global_goal && ((!_node_comparator->isEmpty() && _node_comparator->getBestNode() != _current_goal_node) || _local_goal_obsolete)) || (_pursuing_global_goal && (_global_frontier_obsolete || (!_reached_frontier_goal && _current_goal_node != -1))))
		{
			//		if (!_goal_obsolete) {
			//			_goal_obsolete = true;
			//		ROS_WARN_STREAM(
	...
	void GraphConstructor::updateNodes(geometry_msgs::Point center_node)
	{
		...
		{
			if (_rrg.nodes[iterator.first].status != rrg_nbv_exploration_msgs::Node::EXPLORED && _rrg.nodes[iterator.first].status != rrg_nbv_exploration_msgs::Node::FAILED && _rrg.nodes[iterator.first].status != rrg_nbv_exploration_msgs::Node::INACTIVE && !std::isinf(_rrg.nodes[iterator.first].cost_function))
			{
				_nodes_to_update.push_back(iterator.first);
			}
			if (_rrg.nodes[iterator.first].status != rrg_nbv_exploration_msgs::Node::FAILED && _rrg.nodes[iterator.first].status != rrg_nbv_exploration_msgs::Node::INACTIVE && _rrg.nodes[iterator.first].retry_inflation)
			{ // retry previously failed inflation
				std::vector<int> engulfed_nodes = _collision_checker->inflateExistingNode(_rrg, iterator.first,
	...
	void GraphConstructor::pruneEngulfedNodes(std::vector<int> engulfed_nodes)
	{
		...
		for (auto engulfed_node : engulfed_nodes)
		{
			...
			_rrg.nodes.at(engulfed_node).status = rrg_nbv_exploration_msgs::Node::INACTIVE;
	...
	bool GraphConstructor::compareNodeDistancesToRobot(const int &node_one,
													   const int &node_two)
	{
		return _rrg.nodes.at(node_one).distance_to_robot <= _rrg.nodes.at(node_two).distance_to_robot;
	}
	...
	void GraphConstructor::publishNodeToUpdate()
	{
		...
		else if (_reupdate_nodes && !_nodes_to_reupdate.empty())
		{ // if list is empty, re-update nodes ordered ascending by their distance to the robot
			rrg_nbv_exploration_msgs::NodeToUpdate msg;
			int node = _nodes_to_reupdate.front();
			_nodes_to_reupdate.erase(_nodes_to_reupdate.begin());
			if (_rrg.nodes[node].distance_to_robot > _sensor_range)
			{ // don't re-update nodes outside of sensor range
				_nodes_to_reupdate.clear();
				return;
			}
	...
	void GraphConstructor::handleCurrentLocalGoalFinished()
	{
		...
		case rrg_nbv_exploration_msgs::Node::ABORTED:
			...
			_rrg.nodes[_current_goal_node].status =
				_rrg.nodes[_current_goal_node].gain > 0 ? rrg_nbv_exploration_msgs::Node::INITIAL : rrg_nbv_exploration_msgs::Node::EXPLORED;
			if (_moved_to_current_goal)
			{ // do not update nodes because of constant re-updating
				if (!_reupdate_nodes)
				{
	...
	{
		if (updated_node->index != _last_updated_node || _rrg.nodes[_last_updated_node].gain != updated_node->gain || _rrg.nodes[_last_updated_node].best_yaw != updated_node->best_yaw || _rrg.nodes[_last_updated_node].status != updated_node->status)
		{
			...
			if (updated_node->status != rrg_nbv_exploration_msgs::Node::EXPLORED && updated_node->status != rrg_nbv_exploration_msgs::Node::FAILED && updated_node->status != rrg_nbv_exploration_msgs::Node::INACTIVE)
			{
			...
			else
			{
				...
				if (updated_node->status == rrg_nbv_exploration_msgs::Node::EXPLORED && _current_goal_node == updated_node->index && _node_comparator->isEmpty())
				{ // current goal is explored, change goal
					_local_goal_obsolete = true;
				}
			}
			if (check_waypoints)
						   ...
						   [this](int node)
						   {
							   int collision =
								   _collision_checker->collisionCheckForFailedNode(
									   _rrg, node);
							   if (collision == CollisionChecker::Collisions::unknown)
							   {
								   return false; // keep node and try to recover again
							   }
							   else if (collision == CollisionChecker::Collisions::empty)
							   {
	...
	{
		if (!_pursued_local_goal)
		{ // if a current frontier goal is active, it became obsolete
		...
		rrg_nbv_exploration_msgs::RequestGoal::Response &res)
	{
		...
		{ // local goal
			...
			{
				if (_rrg.nodes[_current_goal_node].status == rrg_nbv_exploration_msgs::Node::VISITED)
				{
					_rrg.nodes[_current_goal_node].status =
						rrg_nbv_exploration_msgs::Node::ACTIVE_VISITED;
				}
				else if (_rrg.nodes[_current_goal_node].status == rrg_nbv_exploration_msgs::Node::INITIAL)
				{
					_rrg.nodes[_current_goal_node].status =
		...
		rrg_nbv_exploration_msgs::UpdateCurrentGoal::Response &res)
	{
		...
		{ // global goal
			if (!_updating_global_goal)
			{
				...
				if (req.status == rrg_nbv_exploration_msgs::Node::VISITED || req.status == rrg_nbv_exploration_msgs::Node::ABORTED)
				{ // reached frontier goal or aborted it for new one
					ROS_INFO_STREAM("Frontier goal reached");
					_reached_frontier_goal = true;
					_consecutive_failed_goals = 0;
				}
				else
				{ // failed to reach frontier goal
