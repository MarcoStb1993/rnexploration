...
GlobalPathWaypointSearcher::GlobalPathWaypointSearcher() :
		_path_kd_tree_index(3, _path_kd_adaptor,
				nanoflann::KDTreeSingleIndexAdaptorParams(20)) {
}

void GlobalPathWaypointSearcher::initialize(
		rrg_nbv_exploration_msgs::GlobalPath &path) {
	_path_kd_adaptor.path = path;
	_path_kd_tree_index.buildIndex();
}

void GlobalPathWaypointSearcher::rebuildIndex(
		rrg_nbv_exploration_msgs::GlobalPath &path) {
	initialize(path);
}

void GlobalPathWaypointSearcher::findNearestNeighbour(
	...
	_path_kd_tree_index.findNeighbors(resultSet, &point_array[0],
			nanoflann::SearchParams(10));
	nearest_node = ret_index;
}

std::vector<std::pair<int, double>> GlobalPathWaypointSearcher::searchInRadius(
	...
	const size_t nMatches = _path_kd_tree_index.radiusSearch(
