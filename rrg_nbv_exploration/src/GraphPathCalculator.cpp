...
{
	GraphPathCalculator::GraphPathCalculator() : _tf_listener(_tf_buffer)
	{
												 ...
												 bool &added_node_to_update)
	{
		...
		if (reset)
		{ // robot moved and is currently at startNode's position
			...
			if (_last_nearest_node >= 0 && findExistingEdge(rrg, start_node, _last_nearest_node) >= 0 && rrg.nodes[_last_nearest_node].status != rrg_nbv_exploration_msgs::Node::FAILED)
			{
												   ...
												   std::list<int> &nodes_to_update, bool &added_node_to_update)
	{
		...
		std::set<std::pair<double, int>> node_queue; // node's cost function (first) and index (second)
		if (distance_to_nearest_node_squared <= _robot_radius_squared)
		{ // robot in proximity to nearest node
			for (int edge : rrg.nodes[start_node].edges)
			{ // find nodes with direct edge to start node and calculate the difference
				...
				{ // reverse yaw when travelling edge the opposite way
					direct_neighbor_node = rrg.edges[edge].first_node;
					edge_yaw =
						rrg.edges[edge].yaw >= 180 ? rrg.edges[edge].yaw - 180 : rrg.edges[edge].yaw + 180;
				}
				if (rrg.nodes[direct_neighbor_node].status != rrg_nbv_exploration_msgs::Node::FAILED)
				{
					...
					rrg.nodes[direct_neighbor_node].distance_to_robot =
						rrg.nodes[start_node].distance_to_robot + rrg.edges[edge].length;
					...
					rrg.nodes[direct_neighbor_node].traversability_cost_to_robot =
						rrg.nodes[start_node].traversability_cost_to_robot + rrg.nodes[direct_neighbor_node].traversability_cost + rrg.edges[edge].traversability_cost;
					rrg.nodes[direct_neighbor_node].traversability_weight_to_robot =
						rrg.nodes[start_node].traversability_weight_to_robot + rrg.nodes[direct_neighbor_node].traversability_weight + rrg.edges[edge].traversability_weight;
					rrg.nodes[direct_neighbor_node].radii_to_robot =
						rrg.nodes[start_node].radii_to_robot + rrg.nodes[direct_neighbor_node].radius;
		...
		else
		{
			node_queue.insert(
				std::make_pair(rrg.nodes.at(start_node).cost_function,
							   start_node));
			if (next_node != -1 && edge_to_next_node != -1 && rrg.nodes[next_node].status != rrg_nbv_exploration_msgs::Node::FAILED)
			{ // ignore start node for distance, path, traversability and radii
													 ...
													 rrg_nbv_exploration_msgs::Graph &rrg)
	{
		int yaw_to_start_node = (int)((atan2(
										   ...
										   rrg.nodes[node].position.x - robot_pos.position.x) *
									   180.0 / M_PI));
			...
			for (auto edge : rrg.nodes[current_node].edges)
			{
				if (!rrg.edges.at(edge).inactive)
				{
					int neighbor_node_index =
						rrg.edges[edge].first_node == current_node ? rrg.edges[edge].second_node : rrg.edges[edge].first_node;
					if (isNodeInPath(neighbor_node_index, current_node, rrg) || rrg.nodes[neighbor_node_index].status == rrg_nbv_exploration_msgs::Node::FAILED)
					{ // don't allow loops and failed nodes in path
						continue;
					}
					...
					if (node_with_new_connection.cost_function < rrg.nodes[neighbor_node_index].cost_function)
					{
						if (!reset && std::isinf(
										  rrg.nodes[neighbor_node_index].cost_function))
						{ // add previously unreachable node to list of nodes to update
	...
	{
		double traversability = ((double)node.traversability_cost_to_robot / (double)_grid_map_occupied) / (double)node.traversability_weight_to_robot;
		...
		double cost_function = _distance_factor * distance + _traversability_factor * traversability + _heading_factor * heading;
		if (_inflation_active && _radius_factor > 0)
		{
			double radius = (node.radii_to_robot / (double)node.path_to_robot.size() / _robot_radius) - 1.0;
			if (cost_function > 0)
		...
		rrg_nbv_exploration_msgs::Graph &rrg)
	{
		...
		updated_node.distance_to_robot = rrg.nodes[neighbor_node].distance_to_robot + rrg.edges[edge].length;
		updated_node.path_to_robot = rrg.nodes[neighbor_node].path_to_robot;
		updated_node.path_to_robot.push_back(node);
		updated_node.radii_to_robot = rrg.nodes[neighbor_node].radii_to_robot + rrg.nodes[node].radius;
		if (rrg.edges[edge].second_node == neighbor_node)
		{ // edge yaw is from first to second node
		...
		updated_node.heading_change_to_robot =
			rrg.nodes[neighbor_node].heading_change_to_robot + getAbsoluteAngleDiff(rrg.nodes[neighbor_node].heading_in,
		...
		updated_node.traversability_cost_to_robot =
			rrg.nodes[neighbor_node].traversability_cost_to_robot + rrg.nodes[node].traversability_cost + rrg.edges[edge].traversability_cost;
		updated_node.traversability_weight_to_robot =
			rrg.nodes[neighbor_node].traversability_weight_to_robot + rrg.nodes[node].traversability_weight + rrg.edges[edge].traversability_weight;
		...
		rrg_nbv_exploration_msgs::Node &node)
	{
		if (node.gain > 0 && _sensor_horizontal_fov < 360)
			return node.heading_change_to_robot + getAbsoluteAngleDiff(node.heading_in, node.best_yaw);
		else
											  ...
											  int node, int neighbor_node)
	{
		...
		{
			if (rrg.edges[edge].first_node == neighbor_node || rrg.edges[edge].second_node == neighbor_node)
	...
	{
		if (rrg.nearest_node == goal_node)
		{ // nearest node to robot and goal node are the same
			getPathFromRobotToPosition(robot_pos, rrg.nodes[goal_node].position,
									   rrg.nodes[goal_node].best_yaw, path);
		}
		else
		{ // build navigation path from path to robot of nearest node to goal node
			...
			{ // compare robot distance to second node on path with edge length between first and second node to decide if first is discarded
				double distance_first_second_squared = pow(
														   rrg.nodes[path_to_robot.at(0)].position.x - rrg.nodes[path_to_robot.at(1)].position.x, 2) +
													   pow(
														   rrg.nodes[path_to_robot.at(0)].position.y - rrg.nodes[path_to_robot.at(1)].position.y,
														   2);
				double distance_second_squared = pow(
													 robot_pos.x - rrg.nodes[path_to_robot.at(1)].position.x, 2) +
												 pow(
													 robot_pos.y - rrg.nodes[path_to_robot.at(1)].position.y,
													 2);
			...
			if (path_to_robot.size() < 2)
			{ // only path from robot to goal node necessary
				getPathFromRobotToPosition(robot_pos, rrg.nodes[goal_node].position,
										   rrg.nodes[goal_node].best_yaw, path);
				return;
			}
			for (auto &i : path_to_robot)
			{ // iterate through nodes in path and add as waypoints for path
				...
				double yaw, distance;
				if (i == path_to_robot.front())
				{ // if node is first element in list, add poses between robot and node
					robot_pos.z = rrg.nodes[i].position.z;
					yaw = atan2(rrg.nodes[i].position.y - robot_pos.y,
								rrg.nodes[i].position.x - robot_pos.x);
					distance = sqrt(
						pow(rrg.nodes[i].position.x - robot_pos.x, 2) + pow(rrg.nodes[i].position.y - robot_pos.y,
																			2));
				...
				if (i != path_to_robot.back())
				{ // if node is not last element in list, get orientation between this node and the next
					int heading;
				...
				else
				{
					...
					{ // use yaw from incoming edge for 360 degrees FoV
						int heading;
		...
		int best_yaw, std::vector<geometry_msgs::PoseStamped> &path)
	{
		...
		if (_sensor_horizontal_fov == 360 || best_yaw == -1)
													 ...
													 int &yaw, double &distance, rrg_nbv_exploration_msgs::Graph &rrg)
	{
		for (int edge : rrg.nodes[start_node].edges)
		{
			if (rrg.edges[edge].first_node == end_node && rrg.edges[edge].second_node == start_node)
			{ // edge yaw is from first to second node
				yaw = rrg.edges[edge].yaw >= 180 ? rrg.edges[edge].yaw - 180 : rrg.edges[edge].yaw + 180;
				distance = rrg.edges[edge].length;
				return true;
			}
			else if (rrg.edges[edge].first_node == start_node && rrg.edges[edge].second_node == end_node)
			{
		...
		geometry_msgs::Point &robot_pos, int closest_waypoint)
	{
		...
		{ // compare robot distance to second waypoint on path with distance between first and second waypoint to decide if first is discarded
			double distance_first_second_squared = pow(
													   waypoints.at(closest_waypoint).x - waypoints.at(closest_waypoint - 1).x, 2) +
												   pow(
													   waypoints.at(closest_waypoint).y - waypoints.at(closest_waypoint - 1).y, 2);
			double distance_second_squared = pow(
		...
		{ // only path from robot to last waypoint
		...
		double distance = sqrt(
			pow(waypoints.at(closest_waypoint).x - robot_pos.x, 2) + pow(waypoints.at(closest_waypoint).y - robot_pos.y, 2));
		quaternion.setRPY(0, 0, yaw);
		quaternion.normalize();
		addInterNodes(path, robot_pos, waypoints.at(waypoints.size() - 1),
					  tf2::toMsg(quaternion), yaw, distance);
		for (int i = closest_waypoint; i >= 0; i--)
		{ // waypoints start at frontier (index 0) and end at closest waypoint to robot
			...
			double yaw, distance;
			if (i != 0)
			{ // if waypoint is not first element in list, get orientation between this waypoint and the previous
				yaw = atan2(waypoints.at(i - 1).y - waypoints.at(i).y,
							waypoints.at(i - 1).x - waypoints.at(i).x);
				distance = sqrt(
					pow(waypoints.at(i).x - waypoints.at(i - 1).x, 2) + pow(waypoints.at(i).y - waypoints.at(i - 1).y,
																			2));
			}
			else
											  ...
											  bool homing)
	{
		...
		if (_sensor_horizontal_fov == 360 || homing)
		{ // use yaw from incoming edge for 360 degrees FoV	or when going back to the start
			double distance;
			if (rrg.nodes[current_goal].path_to_robot.size() < 2 || !getHeadingBetweenNodes(
																		rrg.nodes[current_goal].path_to_robot[rrg.nodes[current_goal].path_to_robot.size() - 2], current_goal, yaw, distance, rrg))
			{ // check if current goal is goal nearest to robot
				yaw = (int)(atan2(
											 ...
											 int startNode, int endNode)
	{
		...
		{
			if ((rrg.edges[edge].first_node == startNode && rrg.edges[edge].second_node == endNode) || (rrg.edges[edge].first_node == endNode && rrg.edges[edge].second_node == startNode))
				return true;
		}
		return false;
	}
		...
		double max_distance_threshold)
	{
		...
		{ // link node copies with connected missing frontiers
			mfwcn += std::to_string(missing_frontier_with_connecting_node.second) + " (" + std::to_string(missing_frontier_with_connecting_node.first) + "),";
		}
		...
		local_nodes.at(frontier_connecting_node).path_to_frontier.push_back(frontier_connecting_node);
		...
		while (!node_queue.empty())
		{
			...
			for (auto edge : rrg.nodes[current_node].edges)
			{
				...
				{ // only traverse active edges
					int neighbor_node_index =
						rrg.edges[edge].first_node == current_node ? rrg.edges[edge].second_node : rrg.edges[edge].first_node;
					double new_length = local_nodes.at(current_node).path_length + rrg.edges.at(edge).length;
					if (new_length <= max_distance_threshold && new_length < local_nodes.at(neighbor_node_index).path_length)
					{ // improved path to neighbor node
						...
						local_nodes.at(neighbor_node_index).path_to_frontier.push_back(neighbor_node_index);
						node_queue.insert(
